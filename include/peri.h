...
typedef int (*per_create_t)(control_unit_t *cu); // Init pointers and peripheral
...
typedef int (*per_gettitle_t)(const char **title);
...
typedef struct {
  ...
  per_gettitle_t gettitle; // Get req title
...
typedef struct {
  peripheral_t pool[PER_MAX_PERIPHERALS];
  int count;
} per_mgr_t;
